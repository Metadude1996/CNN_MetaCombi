...
def plot_confusion_matrix(cm, class_names, savestr, savecm, xlabel=[], ylabel=[]):
    """
    ...
    plt.title(r"$6 \times 6$")
    tick_marks = np.arange(len(class_names))
    if len(xlabel) > 0:
        plt.xticks(tick_marks, xlabel)
    else:
        plt.xticks(tick_marks, class_names)
    if len(ylabel) > 0:
        plt.yticks(tick_marks, ylabel)
    else:
...
def validationset_output_allCNN():
    ...
    dataprek = np.load(u'.\\cnniter_HP_GS_SKF_6x6_AB_5050\\data_prek_xy_train_trainraw_test_6x6.npz')
    ...
    for i in range(N_nf*N_nh*N_lr):
        ...
        for kfold in range(N_kfolds):
            filepath = u"D:\\data\\cnniter_HP_GS_SKF_6x6_AB_5050\\saves\\nf={:d}_nh={:d}_lr={:.4f}\\kfold={:d}\\1\\".format(nf, nh, lr, kfold)
            ...
            ind_trainval = np.load(
                u'D:\\data\\cnniter_HP_GS_SKF_6x6_AB_5050\\saves\\nf={:d}_nh={:d}_lr={:.4f}\\kfold={:d}\\kfold_data_indices.npz'.format(
                    nf, nh, lr, kfold))
            ind_val = ind_trainval['val_index']
            outval = model.predict(x_rest[ind_val])
            np.save('D:\\data\\cnniter_HP_GS_SKF_6x6_AB_5050\\valres_nf{:d}_nh{:d}_lr{:.4f}_kfold{:d}.npy'.format(nf, nh, lr, kfold), outval)
...
def testset_output_allCNN():
    ...
    for i in range(N_nf*N_nh*N_lr):
        ...
        for kfold in range(N_kfolds):
            ...
            np.save('D:\\data\\cnniter_HP_GS_SKF_6x6_AB_5050\\testres_nf{:d}_nh{:d}_lr{:.4f}_kfold{:d}.npy'.format(nf, nh, lr, kfold), outval)
            tf.keras.backend.clear_session()
    return 0
...
def F1score_testset_bestlrCNNs(set_ABdistr = False, AB_frac = 1.0):
    ...
    # return 0
    OPnpz = np.load(
        r'C:\Users\ryanv\PycharmProjects\MetaCombiNN\cnniter_HP_GS_SKF_6x6_AB_5050\np_observables_data.npz')
    ...
    if set_ABdistr:
        np.save(
            u'.\\cnniter_HP_GS_SKF_6x6_AB_5050\\F1_Nsp_Npuc_Npp_Npfpp_lm_Nsprowcol_vs_nf_nh_ABfrac{:.2f}.npy'.format(
                AB_frac), F1OPsmat)
    else:
        np.save(u'.\\cnniter_HP_GS_SKF_6x6_AB_5050\\F1_Nsp_Npuc_Npp_Npfpp_lm_Nsprowcol_vs_nf_nh.npy', F1OPsmat)
    ...
    for op in range(len(OPlabel)):
        if set_ABdistr:
            F1score_heatmap(F1OPsmat[op, :, :, 0], nh_list.astype(int), nf_list.astype(int), '$n_h$', '$n_f$',
                            pdfsave=True,
                            savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\F1score_heatmap_ABfrac{:.2f}'.format(AB_frac)
                                       + OPlabel[op])
            F1score_heatmap(F1OPsmat[op, :, :, 0], nh_list.astype(int), nf_list.astype(int), '$n_h$', '$n_f$',
                            pdfsave=True,
                            savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\F1score_heatmap_ABfrac{:.2f}_vmin0'.format(
                                AB_frac) + OPlabel[op], set_vmin=True, vmin=0)
        else:
            F1score_heatmap(F1OPsmat[op, :, :, 0], nh_list.astype(int), nf_list.astype(int), '$n_h$', '$n_f$',
                            pdfsave=True, savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\F1score_heatmap'+OPlabel[op])
            F1score_heatmap(F1OPsmat[op, :, :, 0], nh_list.astype(int), nf_list.astype(int), '$n_h$', '$n_f$',
                            pdfsave=True, savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\F1score_heatmap_vmin0'
...
def CM_testset():
        ...
        for j, nh in enumerate(nh_list):
            for l, lr in enumerate(lr_list):
                for kfold in range(10):
                    if os.path.isfile('D:\\data\\cnniter_HP_GS_SKF_6x6_AB_5050\\testres_nf{:d}_nh{:d}_lr{:.4f}_kfold{:d}.npy'.format(nf, nh, lr, kfold)):
                        outval = np.load('D:\\data\\cnniter_HP_GS_SKF_6x6_AB_5050\\testres_nf{:d}_nh{:d}_lr{:.4f}_kfold{:d}.npy'.format(nf, nh, lr, kfold))
    ...
    np.save('D:\\data\\cnniter_HP_GS_SKF_6x6_AB_5050\\testset_nfnhlrkfold_TP_FP_TN_FN.npy', CM_mat)
    return 0

def foldavg_heatmap_valacc_and_testCM():
    # resultstring = u'C:\\Users\\ryanv\\PycharmProjects\\MetaCombiNN\\cnniter_HP_GS_SKF_6x6_AB_5050\\kfold_avg_val_results.txt'
    k = 6
    strings = [
        u'C:\\Users\\ryanv\\PycharmProjects\\MetaCombiNN\\cnniter_HP_GS_SKF_6x6_AB_5050\\scratch\\output_dir\\cnniter_HP_GS_SKF_6x6_AB_5050_nh_0to1',
        u'C:\\Users\\ryanv\\PycharmProjects\\MetaCombiNN\\cnniter_HP_GS_SKF_6x6_AB_5050\\scratch\\output_dir\\cnniter_HP_GS_SKF_6x6_AB_5050_nh_1to2',
    ...
    for parts in range(len(strings)):
        ...
        if parts == 0:
            ...
            KFres = np.append(KFres, np.loadtxt(resultstring + u'\\kfold_avg_val_results.txt', delimiter='\t'), axis=0)
    # ConMat = np.loadtxt(r'.\\cnniter_HP_GS_SKF_6x6_AB_5050\\testset_TP_FP_TN_FN_lm.txt', delimiter=',')
    ConMat = np.load(r'D:\\data\cnniter_HP_GS_SKF_6x6_AB_5050\\testset_nfnhlrkfold_TP_FP_TN_FN.npy')
    ...
    F1score_heatmap(PM_nfnh[:, :, 0], nh_list.astype(int), nf_list.astype(int), '$n_h$', '$n_f$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PMscore_heatmap', set_vmin=False, cblabel='acc')
    F1score_heatmap(PM_nfnh[:, :, 0], nh_list.astype(int), nf_list.astype(int), '$n_h$', '$n_f$', pdfsave=True, savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\F1score_heatmap_vmin0', set_vmin=True, cblabel='acc')
    F1score_heatmap(PM_nfnh[:, :, 0], nh_list.astype(int), nf_list.astype(int), '$n_h$', '$n_f$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PMscore_heatmap_vmin0_5', set_vmin=True, vmin=0.5, cblabel='acc')
    F1score_heatmap(PM_nfnh[:, :, 0], nh_list.astype(int), nf_list.astype(int), 'number of hidden neurons', 'number of filters', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PMscore_heatmap_vmin0_5_verbose', set_vmin=True, vmin=0.5, cblabel='acc')
    F1score_heatmap(PM_nfnh[:, :, 0].T, nf_list.astype(int), nh_list.astype(int), 'number of filters',
                    'number of hidden neurons', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PMscore_heatmap_transpose_vmin0_5_verbose',
                    set_vmin=True,
                    vmin=0.5, cblabel='acc', transpose=True)
    F1score_heatmap(PM_nfnh[:, :, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$',
                    '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PMscore_heatmap_transpose_vmin0_5',
                    set_vmin=True,
                    vmin=0.5, cblabel='acc', transpose=True)
    ...
    F1score_heatmap(CM_nfnh[:, :, 0, 0].T/total_Pos, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\ConMat_TP_heatmap_transpose', set_vmin=True, vmin=0, cblabel='TBR', transpose=True)
    F1score_heatmap(CM_nfnh[:, :, 1, 0].T/total_Neg, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\ConMat_FP_heatmap_transpose', set_vmin=True, vmin=0, cblabel='FBR', transpose=True)
    F1score_heatmap(CM_nfnh[:, :, 2, 0].T/total_Neg, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\ConMat_TN_heatmap_transpose', set_vmin=True, vmin=0, cblabel='TAR', transpose=True)
    F1score_heatmap(CM_nfnh[:, :, 3, 0].T/total_Pos, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\ConMat_FN_heatmap_transpose', set_vmin=True, vmin=0, cblabel='FAR', transpose=True)
    ...
    # lr = KFres[arg, 2]
    # string = u'C:\\Users\\ryanv\\PycharmProjects\\MetaCombiNN\\cnniter_HP_GS_SKF_6x6_AB_5050\\logs\\nf={:d}_nh={:d}_lr={:.4f}\\'.format(nf, nh, lr)
    # n_kfolds = 10
    ...
    # print(bestkfold)
    # filepath = r'D:\\data\\cnniter_HP_GS_SKF_6x6_AB_5050\\saves\\nf={:d}_nh={:d}_lr={:.4f}\\kfold={:d}\\1\\'.format(nf, nh, lr, bestkfold)
    # model = tf.keras.models.load_model(
    #     filepath, custom_objects=None, compile=True
    # )
    # model.summary()
    # "load in pre-k train and test data"
    # dataprek = np.load(u'.\\cnniter_HP_GS_SKF_6x6_AB_5050\\data_prek_xy_train_trainraw_test.npz')
    #
    # "remove class C"
    # indA = np.argwhere(dataprek['y_test'] == 0)
    # indB = np.argwhere(dataprek['y_test'] == 1)
    ...
    # plot_confusion_matrix(cm, ['A', 'B'], '.\\cnniter_HP_GS_SKF_6x6_AB_5050\\cm_testset_bestvalF1', True)
    # ind_A = np.argwhere(y_test==0)
    # ind_B = np.argwhere(y_test==1)
    # print(accuracy)
    # density_AnB_NN(out, ind_A, ind_B, '$n_f = ${:d}, $n_h = ${:d}, kfold = {:d} \t $F_1$ score = {:.3f}'.format(nf, nh, bestkfold, f1score), u'.\\cnniter_HP_GS_SKF_6x6_AB_5050\\pdf_testset_truelabel_nf{:d}_nh{:d}_lr{:.4f}_logy'.format(nf, nh, lr), pdfsave=True, logy=True)
    return 0
...
def foldavg_heatmap_valacc_and_testTPR_TNR_BA():
    ...
    F1score_heatmap(PM_nfnh[:, :, 0], nh_list.astype(int), nf_list.astype(int), '$n_h$', '$n_f$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PMscore_heatmap_square', set_vmin=False, cblabel='acc')
    F1score_heatmap(PM_nfnh[:, :, 0], nh_list.astype(int), nf_list.astype(int), '$n_h$', '$n_f$', pdfsave=True, savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\F1score_heatmap_vmin0', set_vmin=True, cblabel='acc')
    F1score_heatmap(PM_nfnh[:, :, 0], nh_list.astype(int), nf_list.astype(int), '$n_h$', '$n_f$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PMscore_heatmap_square_vmin0_5', set_vmin=True, vmin=0.5, cblabel='acc')
    F1score_heatmap(PM_nfnh[:, :, 0], nh_list.astype(int), nf_list.astype(int), 'number of hidden neurons', 'number of filters', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PMscore_heatmap_square_vmin0_5_verbose', set_vmin=True, vmin=0.5, cblabel='acc')
    F1score_heatmap(PM_nfnh[:, :, 0].T, nf_list.astype(int), nh_list.astype(int), 'number of filters',
                    'number of hidden neurons', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PMscore_heatmap_square_transpose_vmin0_5_verbose',
                    set_vmin=True,
                    vmin=0.5, cblabel='acc', transpose=True)
    F1score_heatmap(PM_nfnh[:, :, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$',
                    '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PMscore_heatmap_square_transpose_vmin0_5',
                    set_vmin=True,
                    vmin=0.5, cblabel='acc', transpose=True)
    ...
    F1score_heatmap(testPM_nfnh[:, :, 0, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_TPR_heatmap_square_transpose', set_vmin=True,
                    vmin=0, cblabel='TBR', transpose=True)
    F1score_heatmap(testPM_nfnh[:, :, 1, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_TNR_heatmap_square_transpose', set_vmin=True,
                    vmin=0, cblabel='TAR', transpose=True)
    F1score_heatmap(testPM_nfnh[:, :, 2, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_BA_heatmap_square_transpose', set_vmin=True,
                    vmin=0, cblabel='BA', transpose=True)
    F1score_heatmap(testPM_nfnh[:, :, 3, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_PPV_heatmap_square_transpose', set_vmin=True,
    ...
    F1score_heatmap(testPM_nfnh[:, :, 4, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_NPV_heatmap_square_transpose', set_vmin=True,
    ...
    F1score_heatmap(testPM_nfnh[:, :, 5, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_Pratio_heatmap_square_transpose', set_vmin=True,
    ...
    F1score_heatmap(testPM_nfnh[:, :, 6, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_Oratio_heatmap_square_transpose', set_vmin=True,
                    vmin=0, cblabel='pred O ratio', transpose=True)
    F1score_heatmap(testPM_nfnh[:, :, 5, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_Pratio_logc_heatmap_square_transpose', set_vmin=True,
                    vmin=0.5*10**-2, cblabel='pred P ratio', transpose=True, logc=True)

    F1score_heatmap(testPM_nfnh[:, :, 0, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_TPR_heatmap_square_transpose_nocbar', set_vmin=True,
                    vmin=0, cblabel='TBR', transpose=True, cbar=False)
    F1score_heatmap(testPM_nfnh[:, :, 1, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_TNR_heatmap_square_transpose_nocbar', set_vmin=True,
                    vmin=0, cblabel='TAR', transpose=True, cbar=False)
    F1score_heatmap(testPM_nfnh[:, :, 2, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_BA_heatmap_square_transpose_nocbar', set_vmin=True, vmin=0,
                    cblabel='BA', transpose=True, cbar=False)
    F1score_heatmap(testPM_nfnh[:, :, 3, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_PPV_heatmap_square_transpose_nocbar',
                    set_vmin=True,
                    vmin=0, cblabel='PPV', transpose=True, cbar=False)
    F1score_heatmap(testPM_nfnh[:, :, 4, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_NPV_heatmap_square_transpose_nocbar',
                    set_vmin=True,
                    vmin=0, cblabel='OPV', transpose=True, cbar=False)
    F1score_heatmap(testPM_nfnh[:, :, 5, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_Pratio_heatmap_square_transpose_nocbar',
                    set_vmin=True,
                    vmin=0, cblabel='pred P ratio', transpose=True, cbar=False)
    F1score_heatmap(testPM_nfnh[:, :, 6, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_Oratio_heatmap_square_transpose_nocbar',
                    set_vmin=True,
                    vmin=0, cblabel='pred O ratio', transpose=True, cbar=False)
    F1score_heatmap(testPM_nfnh[:, :, 5, 0].T, nf_list.astype(int), nh_list.astype(int), '$n_f$', '$n_h$', pdfsave=True,
                    savestring='.\\cnniter_HP_GS_SKF_6x6_AB_5050\\test_Pratio_logc_heatmap_square_transpose_nocbar',
                    set_vmin=True,
...
def foldavg_nhlist_valacc_and_testTPR_TNR_PPV_NPV(nf_fix):
    # resultstring = u'C:\\Users\\ryanv\\PycharmProjects\\MetaCombiNN\\cnniter_HP_GS_SKF_6x6_AB_5050\\kfold_avg_val_results.txt'
    k = 6
    strings = [
        u'C:\\Users\\ryanv\\PycharmProjects\\MetaCombiNN\\cnniter_HP_GS_SKF_6x6_AB_5050\\scratch\\output_dir\\cnniter_HP_GS_SKF_6x6_AB_5050_nh_0to1',
    ...
    PM_nfnh = np.zeros((np.shape(nf_list)[0], np.shape(nh_list)[0], 2))
    "CM_nfnh: [nf, nh, {TP, FP, TN, FN}, {avg, var}]"
    testPM_nfnh = np.zeros((np.shape(nf_list)[0], np.shape(nh_list)[0], 9, 2))
    n_lr = 6
    nf_ind = np.argwhere(nf_list == nf_fix)
    for i in range(np.shape(nf_list)[0]):
        for j in range(np.shape(nh_list)[0]):
            # start = i*n_lr
            nfnhargs = np.argwhere(np.logical_and(KFres[:, 0] == nf_list[i], KFres[:, 1] == nh_list[j]))
            ...
            PP_to_TP = np.divide(ConMat[nf_arg, nh_arg, lr_arg, :, 4] + ConMat[nf_arg, nh_arg, lr_arg, :, 5],
                                 ConMat[nf_arg, nh_arg, lr_arg, :, 4] + ConMat[nf_arg, nh_arg, lr_arg, :, 7])
            PN_to_TN = np.divide(ConMat[nf_arg, nh_arg, lr_arg, :, 6] + ConMat[nf_arg, nh_arg, lr_arg, :, 7],
                                 ConMat[nf_arg, nh_arg, lr_arg, :, 5] + ConMat[nf_arg, nh_arg, lr_arg, :, 6])
            ...
            testPM_nfnh[i, j, 6, 0] = np.nanmean(O_ratio)
            testPM_nfnh[i, j, 6, 1] = np.nanvar(O_ratio)
            testPM_nfnh[i, j, 7, 0] = np.nanmean(PP_to_TP)
            testPM_nfnh[i, j, 7, 1] = np.nanvar(PP_to_TP)
            testPM_nfnh[i, j, 8, 0] = np.nanmean(PN_to_TN)
            testPM_nfnh[i, j, 8, 1] = np.nanvar(PN_to_TN)
            # CM_nfnh[i, j, 1, 0], CM_nfnh[i, j, 1, 1] = np.nanmean(ConMat[nf_arg, nh_arg, lr_arg, :, 5]), np.nanvar(ConMat[nf_arg, nh_arg, lr_arg, :, 5])
            # CM_nfnh[i, j, 2, 0], CM_nfnh[i, j, 2, 1] = np.nanmean(ConMat[nf_arg, nh_arg, lr_arg, :, 6]), np.nanvar(ConMat[nf_arg, nh_arg, lr_arg, :, 6])
            # CM_nfnh[i, j, 3, 0], CM_nfnh[i, j, 3, 1] = np.nanmean(ConMat[nf_arg, nh_arg, lr_arg, :, 7]), np.nanvar(ConMat[nf_arg, nh_arg, lr_arg, :, 7])
    ...
    plt.style.use(r'C:/Users/ryanv/PycharmProjects/Matplotlib styles/paper-onehalf.mplstyle')
    f, ax = plt.subplots()
    ax.fill_between(nh_list, testPM_nfnh[nf_ind[0, 0], :, 0, 0] + np.sqrt(testPM_nfnh[nf_ind[0, 0], :, 0, 1]),
                    testPM_nfnh[nf_ind[0, 0], :, 0, 0] - np.sqrt(testPM_nfnh[nf_ind[0, 0], :, 0, 1]),
                    color=cma.colors[1], alpha=0.2)
    ax.plot(nh_list, testPM_nfnh[nf_ind[0, 0], :, 0, 0], '.-', c=cma.colors[0], label='TPR')
    ax.fill_between(nh_list, testPM_nfnh[nf_ind[0, 0], :, 1, 0] + np.sqrt(testPM_nfnh[nf_ind[0, 0], :, 1, 1]),
                    testPM_nfnh[nf_ind[0, 0], :, 1, 0] - np.sqrt(testPM_nfnh[nf_ind[0, 0], :, 1, 1]),
                    color=cma.colors[5], alpha=0.2)
    ax.plot(nh_list, testPM_nfnh[nf_ind[0, 0], :, 1, 0], '.-', c=cma.colors[4], label='TOR')
    ax.fill_between(nh_list, testPM_nfnh[nf_ind[0, 0], :, 3, 0] + np.sqrt(testPM_nfnh[nf_ind[0, 0], :, 3, 1]),
                    testPM_nfnh[nf_ind[0, 0], :, 3, 0] - np.sqrt(testPM_nfnh[nf_ind[0, 0], :, 3, 1]),
                    color=cma.colors[9], alpha=0.2)
    ax.plot(nh_list, testPM_nfnh[nf_ind[0, 0], :, 3, 0], '.-', c=cma.colors[8], label='PPV')
    ax.fill_between(nh_list, testPM_nfnh[nf_ind[0, 0], :, 4, 0] + np.sqrt(testPM_nfnh[nf_ind[0, 0], :, 4, 1]),
                    testPM_nfnh[nf_ind[0, 0], :, 4, 0] - np.sqrt(testPM_nfnh[nf_ind[0, 0], :, 4, 1]),
                    color=cma.colors[13], alpha=0.2)
    ax.plot(nh_list, testPM_nfnh[nf_ind[0, 0], :, 4, 0], '.-', c=cma.colors[12], label='OPV')

    ax.set_xlabel('$n_h$')
    plt.legend()
    plt.tight_layout()
    plt.savefig('.\\cnniter_HP_GS_SKF_6x6_AB_5050\\TPR_TNR_PPV_NPV_vs_nh_nf{:d}.pdf'.format(nf_fix),
                facecolor = f.get_facecolor())
    plt.savefig('.\\cnniter_HP_GS_SKF_6x6_AB_5050\\TPR_TNR_PPV_NPV_vs_nh_nf{:d}.svg'.format(nf_fix),
                facecolor=f.get_facecolor())
    plt.savefig('.\\cnniter_HP_GS_SKF_6x6_AB_5050\\TPR_TNR_PPV_NPV_vs_nh_nf{:d}.png'.format(nf_fix),
                dpi=400, facecolor=f.get_facecolor())
    plt.show()
    plt.close()

    f, ax = plt.subplots()
    ax.fill_between(nh_list, testPM_nfnh[nf_ind[0, 0], :, 7, 0] + np.sqrt(testPM_nfnh[nf_ind[0, 0], :, 7, 1]),
                    testPM_nfnh[nf_ind[0, 0], :, 7, 0] - np.sqrt(testPM_nfnh[nf_ind[0, 0], :, 7, 1]),
                    color=cma.colors[1], alpha=0.2)
    ax.plot(nh_list, testPM_nfnh[nf_ind[0, 0], :, 7, 0], '.-', c=cma.colors[0], label='PP_to_TP')
    ax.fill_between(nh_list, testPM_nfnh[nf_ind[0, 0], :, 8, 0] + np.sqrt(testPM_nfnh[nf_ind[0, 0], :, 8, 1]),
                    testPM_nfnh[nf_ind[0, 0], :, 8, 0] - np.sqrt(testPM_nfnh[nf_ind[0, 0], :, 8, 1]),
                    color=cma.colors[5], alpha=0.2)
    ax.plot(nh_list, testPM_nfnh[nf_ind[0, 0], :, 8, 0], '.-', c=cma.colors[4], label='PO_to_TO')
    ax.set_xlabel('$n_h$')
    plt.legend()
    plt.tight_layout()
    plt.savefig('.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PP_to_TP_PO_to_TO_vs_nh_nf{:d}.pdf'.format(nf_fix),
                facecolor=f.get_facecolor())
    plt.savefig('.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PP_to_TP_PO_to_TO_vs_nh_nf{:d}.svg'.format(nf_fix),
                facecolor=f.get_facecolor())
    plt.savefig('.\\cnniter_HP_GS_SKF_6x6_AB_5050\\PP_to_TP_PO_to_TO_vs_nh_nf{:d}.png'.format(nf_fix),
                dpi=400, facecolor=f.get_facecolor())
    plt.show()
    plt.close()
...
def test_random_walk(k):
    ...
    print('best k-fold:')
    print(bestkfold)
    filepath = u"D:\\data\\cnniter_HP_GS_SKF_6x6_AB_5050\\saves\\nf={:d}_nh={:d}_lr={:.4f}\\kfold={:d}\\1\\".format(nf,
                                                                                                                    nh,
                                                                                                                    lr,
                                                                                                                    bestkfold)
    model = tf.keras.models.load_model(
    ...
    model.summary()
    Nsteps = k * k
    PtoO = np.zeros(Nsteps, dtype=int)
    PtoP = np.zeros(Nsteps, dtype=int)
    OtoO = np.zeros(Nsteps, dtype=int)
    OtoP = np.zeros(Nsteps, dtype=int)
    batch_size = 5000
    for j, l in enumerate(range(0, 150000, batch_size)):
        ...
        for i in range(batch_size):
            ...
            ind_P = np.argwhere(y_pred[ind_l:ind_r] == 1)
            ind_O = np.argwhere(y_pred[ind_l:ind_r] == 0)
            if y_pred[ind_0] == 1:
                'predicted P'
                PtoP[ind_P[:, 0]] += 1
                PtoO[ind_O[:, 0]] += 1
                ...
                OtoP[ind_P[:, 0]] += 1
                OtoO[ind_O[:, 0]] += 1
    p_OtoO = np.divide(OtoO, np.add(OtoO, OtoP))
    p_OtoP = 1 - p_OtoO
    p_PtoP = np.divide(PtoP, np.add(PtoP, PtoO))
    p_PtoO = 1 - p_PtoP
    np.savez(savedir + 'nf{:d}_nh{:d}_lr{:.4f}_kfold{:d}_probability_classchange_randomwalk_{:d}x{:d}_test_.npz'.format(
        nf, nh, lr, bestkfold, k, k), OtoO=OtoO,
             OtoP=OtoP, PtoO=PtoO, PtoP=PtoP, p_OtoO=p_OtoO, p_PtoP=p_PtoP, p_OtoP=p_OtoP, p_PtoO=p_PtoO)
    f, ax = plt.subplots()
    ax.plot(np.arange(1, Nsteps + 1), p_OtoP, c='r', label='O to P')
    ax.plot(np.arange(1, Nsteps + 1), p_PtoP, c='g', label='P to P')
    ax.set_xlabel('steps')
    ax.set_ylabel('p')
