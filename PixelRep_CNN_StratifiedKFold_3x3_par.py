...
"train convolutional neural networks to classify 3x3 unit cells into class C or I"
import sys
...
# ind_data = np.arange(np.shape(npz['y_rest'])[0], dtype=int)
# np.random.shuffle(ind_data)
# N_data = len(ind_data)
# N_datanew = int(0.5*N_data)
# x_rest = npz['x_rest'][ind_data[:N_datanew]].astype(np.float32)
# y_rest = npz['y_rest'][ind_data[:N_datanew]].astype(np.float32)
