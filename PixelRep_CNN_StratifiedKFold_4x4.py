...
"train convolutional neural networks to classify 4x4 unit cells into class C or I"
...
# np.savez(os.path.join(masterdir, 'data_prek_xy_train_trainraw_test_4x4.npz'), x_rest=x_rest, y_rest=y_rest, x_rest_raw=x_rest_raw, y_rest_raw=y_rest_raw, x_test=x_test, y_test=y_test)

npz = np.load(os.path.join(InputDataDirectory, r'data_prek_xy_train_trainraw_test_4x4.npz'))
x_rest = npz['x_rest']
y_rest = npz['y_rest']
    ...
    for n_hidden in HP_NUM_HIDDEN:
        ...
        for learning_rate in HP_LEARNING_RATE:
            ...
            for train_index, val_index in skf.split(x_rest, y_rest):
                ...
                train_ds = tf.data.Dataset.from_tensor_slices((x_rest[train_index], y_rest[train_index])).shuffle(BUFFER_SIZE).batch(GLOBAL_BATCH_SIZE).prefetch(BUFFER_SIZE*4)

                val_ds = tf.data.Dataset.from_tensor_slices((x_rest[val_index], y_rest[val_index])).batch(GLOBAL_BATCH_SIZE).prefetch(len(val_index)*4)
